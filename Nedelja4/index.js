...
removeBtn.addEventListener('click', removeSelcted);

invertBtn.addEventListener('click', selectInverted);


function removeSelcted() {
    let checktask = document.querySelectorAll('.chk-state');
    checktask.forEach(e => {
        if (e.checked) e.parentElement.parentElement.remove();
    });
}
...
function addItem() {
    ...
    itemText.innerHTML = `${text} (${level.value})`;    
